...
def generate_btor2kw(benchmark_path: str) -> list[int]:
    ...
    if not Path(BTOR2KW_BINARY).is_file():
        raise ValueError(
            "The btor2kwcount binary does not exist. Please build it first."
        )
    btor2kw_output = subprocess.run(
        [BTOR2KW_BINARY, benchmark_path], capture_output=True, text=True
    )
    ...
    if len(kw_counts) != len(KEYWORDS):
        raise ValueError(
            f"Unexpected number {len(kw_counts)} of keywords in {benchmark_path} embedding"
        )
    return kw_counts
